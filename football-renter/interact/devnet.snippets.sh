...
set_minimum_deposit() {  
    ...
    mxpy --verbose contract call ${ADDRESS} --pem=${GABI} --gas-limit 100000000 --function="setMinDeposit" --arguments ${AMOUNT} --proxy=${PROXY} --send  
}  
...
create_slot() {  
    ...
    mxpy --verbose contract call ${ADDRESS} --pem=${GABI} --gas-limit 100000000 --function="create_football_slot" --arguments ${START_TIME} ${END_TIME} --value=${EGLD_AMOUNT} --gas-limit=10000000  --proxy=${PROXY} --send  
}  
